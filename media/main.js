...
window.addEventListener('load', () => {
    ...
    console.log(
        'Available functions:',
        Object.keys(window).filter((key) => typeof window[key] === 'function' && key.includes('Commit')),
    );
...
function updateCommitHistory() {
    ...
    historyContainer.innerHTML = currentData.commitHistory
        .map(
            (commit, index) =>
                ...
                '<button class="small-button" onclick="editCommit(' +
                index +
                ')">Edit</button>' +
                '<button class="small-button" onclick="removeCommit(' +
                index +
                ')">Remove</button>' +
                '</div>' +
                '</div>',
...
function showError(errorInfo) {
    ...
    if (errorActions && errorInfo.suggestedActions) {
        errorActions.innerHTML = errorInfo.suggestedActions.map((action) => '<div class="error-action">' + escapeHtml(action) + '</div>').join('');
    }
...
function renderCommitPlanSummary() {
    ...
    if (plan.warnings && plan.warnings.length > 0) {
        warningsHtml = plan.warnings.map((warning) => '<div class="warning-item">⚠️ ' + escapeHtml(warning) + '</div>').join('');
    }
    ...
    summaryContainer.innerHTML =
        ...
        '<div class="stat-item">' +
        '<div class="stat-value">' +
        Math.ceil(plan.estimatedTime / 60) +
        'm</div>' +
        ...
        .map((group, index) => {
            const filesHtml = group.files
                .map((file) => {
                    ...
                    return (
                        '<div class="file-item">' +
                        '<span class="file-change-type change-type-' +
                        file.changeType +
                        '">' +
            ...
            return (
                ...
                '<span class="commit-type-badge commit-type-' +
                group.type +
                '">' +
                group.type +
                '</span>' +
                '<div class="commit-message" onclick="editCommitMessage(\'' +
                safeGroupId +
                "', '" +
                safeMessage +
                '\')">' +
                ...
                '</div>' +
                '<div class="commit-group-stats">' +
                group.fileCount +
                ' files</div>' +
                '</div>' +
                '<div class="commit-group-files">' +
                filesHtml +
                '</div>' +
                '</div>'
            );
